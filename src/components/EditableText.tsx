...
export const EditableText: React.FC<EditableTextProps> = ({
  ...
  const currentText = customTexts?.[id] !== undefined ? customTexts[id] : defaultText;
  const [editText, setEditText] = useState(currentText);

  const handleSave = () => {
    if (onSaveText) {
      onSaveText(id, editText);
    }
    setIsOpen(false);
  };
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="inline-flex items-center justify-center w-4 h-4 rounded bg-amber-500 text-white hover:bg-amber-600 transition-colors shadow-2xs shrink-0"
